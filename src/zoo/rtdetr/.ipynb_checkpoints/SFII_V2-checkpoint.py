...
from ...nn.backbone.common import get_activation, FrozenBatchNorm2d
...
class BidomainNonlinearMapping(nn.Module):
    ...
    def __init__(self, in_nc):
        ...
        from .hybrid_encoder import EdgeCspRepLayer, CSPRepLayer, ConvNormLayer
        self.spatial_process = ConvNormLayer(4, 64, 1, 1, padding=0, act=None, use_wt=False)

        self.freqConv = ConvNormLayer(in_nc, 64, 1, 1, padding=0, act=None, use_wt=False)
        self.frequency_process = Frequency_Spectrum_Dynamic_Aggregation(64)


        self.bottlenecks = CSPRepLayer(128, 64, expansion=3, num_blocks=3, useglu=False)
        ...
        self.edgeConv = nn.Sequential(OrderedDict([
            (name, ConvNormLayer(cin, cout, k, s, act=act)) for cin, cout, k, s, name in conv_def_4
        ]))
        ...
        with torch.no_grad():
            for idx, item in enumerate(self.conv1):
                if idx == 0:
                    conv1[idx].conv.weight[:, :3, :, :] = self.conv1[idx].conv.weight.clone()
                    conv1[idx].conv.weight[:, 3, :, :] = self.conv1[idx].conv.weight.clone()[:, 0, :, :]
                else:
                    conv1[idx].conv.weight.data = self.conv1[idx].conv.weight.clone()
                # 复制归一化层的权重和偏置
                conv1[idx].norm.weight.data = self.conv1[idx].norm.weight.data.clone()
                conv1[idx].norm.bias.data = self.conv1[idx].norm.bias.data.clone()
            self.edgeConv = conv1
        # self.edgeConv.load_state_dict(new_state_dict)
        
        print(f'加载了PResNet的conv1的state_dict')
        self._freeze_parameters(self.conv1)
        # self._freeze_parameters(self.edgeConv)
        self._freeze_norm(self.conv1)
        self._freeze_norm(self.edgeConv)
    ...
    def forward(self, x):
        ...
        x_freq_spatial = self.conv1(x)
        x_freq_spatial = F.max_pool2d(x_freq_spatial, kernel_size=3, stride=2, padding=1)
        x_freq_spatial = self.frequency_process(x_freq_spatial)
        
        xcat = torch.concat([x1, x_freq_spatial], 1)
        xcat = self.bottlenecks(xcat)

        x_out = self.sigmoid(xcat) * x1
        # print(xout.shape)
        return x_out
