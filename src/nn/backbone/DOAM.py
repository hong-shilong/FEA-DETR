...
class GatedConv2dWithActivation(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
        ...
        self.conv2d = torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias).to(torch.device('cuda'))
        self.mask_conv2d = torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups,
                                           bias).to(torch.device('cuda'))
        self.batch_norm2d = torch.nn.BatchNorm2d(out_channels).to(torch.device('cuda'))
...
class DOAM(nn.Module):
    def __init__(self):
        ...
        self.gamma = nn.Parameter(torch.zeros(1)).to(torch.device('cuda'))
        ...
        self.conv2d_1_1.add_module('conv2d_1_1', nn.Conv2d(9, 1, kernel_size=3, padding=1).to(torch.device('cuda')))
        ...
        self.conv2d_1_attention.add_module('conv2d_1_attention', nn.Conv2d(1, 8, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_2_attention = torch.nn.Sequential()
        self.conv2d_2_attention.add_module('conv2d_2_attention', nn.Conv2d(8, 16, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_3_attention = torch.nn.Sequential()
        self.conv2d_3_attention.add_module('conv2d_3_attention', nn.Conv2d(16, 16, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_4_attention = torch.nn.Sequential()
        self.conv2d_4_attention.add_module('conv2d_4_attention', nn.Conv2d(16, 8, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_5_attention = torch.nn.Sequential()
        self.conv2d_5_attention.add_module('conv2d_5_attention', nn.Conv2d(8, 1, kernel_size=3, padding=1).to(torch.device('cuda')))
        #self.conv2d_2_attention = nn.Conv2d(16, 1, kernel_size=3, padding=1)

        #rgb convolution
        self.conv2d_1_rgb_attention = torch.nn.Sequential()
        self.conv2d_1_rgb_attention.add_module('conv2d_1_rgb_attention', nn.Conv2d(4, 8, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_2_rgb_attention = torch.nn.Sequential()
        self.conv2d_2_rgb_attention.add_module('conv2d_2_rgb_attention', nn.Conv2d(8, 16, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_3_rgb_attention = torch.nn.Sequential()
        self.conv2d_3_rgb_attention.add_module('conv2d_3_rgb_attention', nn.Conv2d(16, 32, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_4_rgb_attention = torch.nn.Sequential()
        self.conv2d_4_rgb_attention.add_module('conv2d_4_rgb_attention', nn.Conv2d(32, 16, kernel_size=3, padding=1).to(torch.device('cuda')))
        self.conv2d_5_rgb_attention = torch.nn.Sequential()
        self.conv2d_5_rgb_attention.add_module('conv2d_5_rgb_attention', nn.Conv2d(16, 8, kernel_size=3, padding=1).to(torch.device('cuda')))
        ...
        self.conv2d_1_rgb_red_concat_5.add_module('conv2d_1_rgb_red_concat_5',
                                                  nn.Conv2d(16, 8, kernel_size=3, padding=1).to(torch.device('cuda')))
    ...
    def RIA(self, x):
        ...
        og_x = x
        refine_30 = []
        x_pooled_upsample_5 = torch.zeros(x.shape, device=x.device)
        x_pooled_upsample_10 = torch.zeros(x.shape, device=x.device)
        x_pooled_upsample_15 = torch.zeros(x.shape, device=x.device)
    ...
    def forward(self, im):
        ...
        # save_image(edge_detect, './img/inputDOAMImgChannel0.png')
...
if __name__ == '__main__':
    # x = torch.randn(4, 3, 640, 640)*255
    x = torch.load('./img.pt').to(torch.device("cuda"))
    print(x.shape)
    B, C, H, W = x.shape
    attention = DOAM()
    x0 = attention(x)
    print('output shape:' + str(x0.shape))
