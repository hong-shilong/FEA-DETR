...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def __getitem__(self, idx):
        ...
        assert isinstance(img, (np.ndarray, Image.Image)), f"Unexpected img type: {type(img)}, idx:{idx}, imageid:{self.ids[idx]}"
        if self._transforms is not None:
            # print(f"img type: {type(img)}, idx:{idx}, imageid:{self.ids[idx]}")
